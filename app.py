                        ...
                        if len(data) > 0:
                            ...
                            if len(inputs["symbols"]) > 1:
                                ...
                                try:
                                    ...
                                    if portfolio_stats:
                                        # Display portfolio metrics (only numeric values, not the time series data)
                                        metric_keys = [k for k in portfolio_stats.keys() if k not in ['dates', 'values']]
                                        portfolio_cols = st.columns(len(metric_keys))
                                        
                                        for i, metric in enumerate(metric_keys):
                                            value = portfolio_stats[metric]
