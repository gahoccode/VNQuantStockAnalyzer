...
def create_portfolio_comparison_chart(portfolio_data, benchmark_data=None):
    ...
    Args:
        portfolio_data: Dictionary with portfolio performance metrics and time series data
    ...
    """
    try:
        # Verify that portfolio_data contains the necessary time series data
        if 'dates' not in portfolio_data or 'values' not in portfolio_data:
            print("Portfolio data missing required time series data (dates/values)")
            return None
        ...
        if benchmark_data and 'dates' in benchmark_data and 'values' in benchmark_data:
            fig.add_trace(go.Scatter(
